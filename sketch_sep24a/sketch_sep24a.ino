...
void setup()
{
  myLcd.begin(16,2);
  Serial.begin(9600);
    programTemp = 0; // en el inicio no tiene temperatura programada
    status.machineState = IDLE;    // la máquina arranca en estado cero.
    tempOut = map(analogRead(A1),20,358,-40,125); // leo la temperatura del sensor y la mapeo como entero
